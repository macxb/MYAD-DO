...
exports.run = async (client, message) => {
  ...
  if (!voiceChan || voiceChan.type !== "voice") {
    message.channel.send("```No estas conectado a un canal de voz.```").catch(error => console.log(error.stack));
  } else if (clientVcheck) {
      message.channel.send("```No estoy conectado a un canal de voz.```").catch(error => console.log(error.stack));
    } else {
